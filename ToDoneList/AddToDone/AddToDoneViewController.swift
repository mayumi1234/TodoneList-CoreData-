...
class AddToDoneViewController: UIViewController {
    ...
    private func createToFirestore() {
        ...
        // 日付をデータとして記録
        db.collection("users").document(uid).collection("date").document(date).setData(documentDate) { (err) in
            if let err = err {
        ...
        // タスクを記録
        db.collection("users").document(uid).collection("date").document(date).collection("tasks").document(documentID).setData(docData) { (err) in
            if let err = err {
                print("Firestoreへの保存に失敗しました。\(err)")
                HUD.hide()
                HUD.flash(.labeledError(title: "失敗しました。", subtitle: "\(err)"), delay: HUDTime)
